...
export const endpoints = {
  ...
  list: ({ limit = ITEMS_PER_PAGE, page_token }: ListParams) =>
    `${env.VITE_API_BASE_URL}/user/text-to-cad?no_models=true&limit=${limit}${page_token ? `&page_token=${page_token}` : ''
    }`,
  ...
  viewNoModels: (id: string) =>
    `${env.VITE_API_BASE_URL}/user/text-to-cad/${id}?no_models=true`,
